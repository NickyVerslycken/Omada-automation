...
class App(tk.Tk):
    def __init__(self):
        ...
        self.current_gateway_filter_index: int = -1  # -1 means All
        self.batch_gateway_index: int = -1
        ...
        self._networks_cache_all: List[Dict[str, Any]] = []
    ...
    def _build_current_tab(self):
        ...
        self.var_gateway_current = tk.StringVar()
        self.cmb_gateways_current = ttk.Combobox(top, textvariable=self.var_gateway_current, width=40, state="disabled")
        self.cmb_gateways_current.pack(side="left")
        self.cmb_gateways_current.bind("<<ComboboxSelected>>", self.on_gateway_selected_current)

        self.btn_refresh_gateways_current = ttk.Button(
            top, text="Refresh DHCP servers", command=self.on_refresh_gateways, state="disabled"
        )
        self.btn_refresh_gateways_current.pack(side="left", padx=(8, 0))
    ...
    def _build_batch_tab(self):
        ...
        ttk.Label(form, text="DHCP server").grid(row=r, column=0, sticky="w", pady=(6, 0))
        self.var_gateway_batch = tk.StringVar()
        self.cmb_gateways_batch = ttk.Combobox(form, textvariable=self.var_gateway_batch, width=46, state="disabled")
        self.cmb_gateways_batch.grid(row=r, column=1, columnspan=2, sticky="w", pady=(6, 0))
        self.cmb_gateways_batch.bind("<<ComboboxSelected>>", self.on_gateway_selected_batch)

        self.btn_refresh_gateways_batch = ttk.Button(
            form, text="Refresh DHCP servers", command=self.on_refresh_gateways, state="disabled"
        )
        self.btn_refresh_gateways_batch.grid(row=r, column=3, sticky="w", pady=(6, 0))
        r += 1
    ...
    def _on_gateways(self, gateways):
        ...
        current_values = ["All"] + labels
        self.cmb_gateways_current["values"] = current_values
    ...
    def _batch_gateway_name(self) -> Optional[str]:
        g = self._batch_gateway()
        if not g:
            return None
        return str(g.get("name") or "").strip() or None

    def on_gateway_selected_current(self, _evt=None):
        idx = self.cmb_gateways_current.current()
        self.current_gateway_filter_index = idx - 1
        g = self._current_gateway()
        if g:
            self._q.put(("log", f"Current-tab DHCP filter={g.get('name') or g.get('label') or '(unnamed)'}"))
        else:
            self._q.put(("log", "Current-tab DHCP filter=All"))
        self._apply_network_filter()

    def on_gateway_selected_batch(self, _evt=None):
        idx = self.cmb_gateways_batch.current()
        self.batch_gateway_index = idx
        g = self._batch_gateway()
        if g:
            self._q.put(("log", f"Batch DHCP server={g.get('name') or g.get('label') or '(unnamed)'}"))
        self._update_push_state()
    ...
    def _apply_network_filter(self):
        ...
        if selected is None:
            ...
            if not self._networks_cache and self._networks_cache_all:
                # Some controllers do not expose DHCP-server ownership fields in LAN network payloads.
                # In that case, keep data visible instead of showing an empty list.
                self._networks_cache = list(self._networks_cache_all)
                self._q.put(("log", "WARNING: Could not map networks to selected DHCP server; showing all networks."))
    ...
    def _network_matches_gateway(self, network: Dict[str, Any], gateway: Dict[str, Any]) -> bool:
        ...
        gateway_tokens = {
            str(gateway.get("device_id") or "").strip().lower(),
            str(gateway.get("value") or "").strip().lower(),
            str(gateway.get("name") or "").strip().lower(),
            str(gateway.get("mac") or "").strip().lower(),
        }
        gateway_tokens.discard("")

        net_tokens = {
            str(network.get("gateway") or "").strip().lower(),
            str(network.get("gatewayId") or "").strip().lower(),
            str(network.get("gatewayDeviceId") or "").strip().lower(),
            str(network.get("gatewayName") or "").strip().lower(),
            str(network.get("dhcpServer") or "").strip().lower(),
            str(network.get("dhcpServerId") or "").strip().lower(),
            str(network.get("serverId") or "").strip().lower(),
        }
        dhcp = network.get("dhcpSettingsVO") or {}
        if isinstance(dhcp, dict):
            net_tokens.update(
                {
                    str(dhcp.get("gateway") or "").strip().lower(),
                    str(dhcp.get("gatewayId") or "").strip().lower(),
                    str(dhcp.get("dhcpServer") or "").strip().lower(),
                    str(dhcp.get("dhcpServerId") or "").strip().lower(),
                }
            )
        net_tokens.discard("")
        ...
        } if isinstance(network.get("interfaceIds"), list) else set()
