...
class OmadaOpenApiClient:
    ...
    def get_site_gateways(self, site_id: str, page: int = 1, page_size: int = 500) -> List[Dict[str, str]]:
        if not self.omadac_id:
            raise RuntimeError("omadacId unknown.")
        ...
        explicit_gateways: List[Dict[str, str]] = []
        unknown_gateways: List[Dict[str, str]] = []
        seen: set[str] = set()
        ...
        for dev in devices:
            ...
            entry = {"name": name, "label": label, "value": value}
    ...
    def create_lan_network(self, site_id: str, plan: PlannedLan, gateway_device: Optional[str] = None) -> Dict[str, Any]:
        if not self.omadac_id:
            raise RuntimeError("omadacId unknown.")
        url = f"{self.base_url}/openapi/v2/{self.omadac_id}/sites/{site_id}/lan-networks"
...
class App(tk.Tk):
    ...
    def on_push_plan(self):
        if not self.client or not self.selected_site_id:
            messagebox.showwarning("Missing", "Connect and select a site first.")
            return
        if not self.selected_gateway_value:
        ...
        def work():
            ...
            assert site_id
            gateway_value = self.selected_gateway_value
            ...
            for p in self.plan:
                ...
                resp = self.client.create_lan_network(site_id, p, gateway_device=gateway_value)
    ...
    def _update_push_state(self):
        enabled = bool(self.client and self.selected_site_id and self.selected_gateway_value and len(self.plan) > 0)
